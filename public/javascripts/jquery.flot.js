...
(function($) {
    function Plot(target_, data_, options_) {
        ...
        var lastMousePos = { pageX: null, pageY: null, dirX: 0 };
        ...
        function setSelectionPos(pos, e) {
            ...
            if (options.selection.mode == "y") {
                ...
                if (glob == true) {
                    var __x = xaxis.min + pos.x / hozScale;
                    var idx = pos == selection.first ? -1 : 0

                    /*
                    if (pos == selection.second &&
                        pos.x > -1 &&
                        pos.x < selection.first.x) {
                        //selection.first.x = selection.second.x;
                        //idx = -1
                        console.log("px:" + pos.x + ",fx:" + selection.first.x);
                    }
                    */
                    for(var i=0; i < xaxis.ticks.length; i++) {
                        if ( __x < xaxis.ticks[i].v) {
                            pos.x = Math.floor((xaxis.ticks[i+idx].v - xaxis.min) * hozScale);
                            break;
                        }
                    }

                    console.log(selection.first.x + "," + selection.first.y + "::" + selection.second.x + "," + selection.second.y)
                }
            }
